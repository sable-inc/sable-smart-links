...
const TEXT_AGENT_GUIDE_CLASS = 'sable-text-agent-guide';
const TEXT_AGENT_GUIDE_CONTAINER_ID = 'sable-text-agent-guide-container';

// Keep track of active tooltip
let activeTextAgentGuide = null;
...
function injectTooltipStyles() {
  ...
  if (safeDocument.getElementById('sable-text-agent-guide-styles')) {
    return;
  }
  
  const styleElement = safeDocument.createElement('style');
  styleElement.id = 'sable-text-agent-guide-styles';
  styleElement.textContent = `
    .${TEXT_AGENT_GUIDE_CLASS} {
      position: absolute;
    ...
    .${TEXT_AGENT_GUIDE_CLASS}-content {
      margin: 0 0 16px 0;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-buttons {
      display: flex;
      justify-content: flex-end;
      gap: 8px;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-button {
      padding: 8px 16px;
      border-radius: 4px;
      font-weight: 500;
      cursor: pointer;
    ...
    .${TEXT_AGENT_GUIDE_CLASS}-button-primary {
      background-color: #3498db;
      color: white;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-button-primary:hover {
      background-color: #2980b9;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-button-secondary {
      background-color: #f1f1f1;
      color: #333333;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-button-secondary:hover {
      background-color: #e1e1e1;
    }
    
    .${TEXT_AGENT_GUIDE_CLASS}-floating {
      position: fixed;
      top: 50%;
      left: 50%;
    ...
    @keyframes sable-text-agent-guide-fade-in {
      from {
        opacity: 0;
 ...
 */
function getTextAgentGuideContainer() {
  // Only run in browser environment
  if (!isBrowser) return null;
  
  let container = safeDocument.getElementById(TEXT_AGENT_GUIDE_CONTAINER_ID);
  
  if (!container) {
    container = safeDocument.createElement('div');
    container.id = TEXT_AGENT_GUIDE_CONTAINER_ID;
